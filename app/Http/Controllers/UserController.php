...
class UserController extends Controller {
 ...
 public function create(){ return view('users.form',['user'=>new User(),'roles'=>request()->user()->isAdmin()?['admin','petugas','customer']:['petugas','customer']]); }
  public function store(StoreUserRequest $request){ $data=$request->safe()->all(); $data['password']=Hash::make($request->password); $user=User::create($data); return redirect()->route('users.index')->with('success','Data berhasil ditambahkan.'); }
